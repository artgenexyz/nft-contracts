// SPDX-License-Identifier: MIT
pragma solidity ^0.8.0;

import "../Artgene721Base.sol";

contract RIPPLES is Artgene721Base {
    constructor()
        Artgene721Base(
            "Ripples",
            "RIPPLES",
            ARTGENE_MAX_SUPPLY_OPEN_EDITION,
            10,
            false,
            "https://metadata.artgene.xyz/api/g/era/ripples/",
            MintConfig(
                0.00025 ether, // public price
                10, // maxTokensPerMint,
                10, // maxTokensPerWallet,
                500, // basis points royalty fee
                0x00007d9F58AF8115A3C8FEfeD555E30F972EACEd, // payout receiver
                false, // should lock payout receiver
                1689955200, // startTimestamp
                1690041600 // endTimestamp
            )
        )
    {
        // opensea proxy is disabled on zksync
        setIsOpenSeaProxyActive(false);

        transferOwnership(0xffE06cb4807917bd79382981f23d16A70C102c3B);
    }

    function _startTokenId() internal view virtual override returns (uint256) {
        return 1;
    }

}
