...
export async function deployContract(
  ...
  console.log(
    ...
    `\n| gasPerPubdata\t\t| ${(
      tx as any
    ).customData.gasPerPubdata.toString()}\t\t\t\t|`,
    `\n| gasLimit\t\t| ${tx.gasLimit.toString()}\t\t\t|`,
    `\n| gasUsed\t\t| ${receipt.gasUsed.toString()}\t\t\t|`,
    `\n| maxPriorityFeePerGas\t| ${tx.maxPriorityFeePerGas?.toString()}\t\t\t|`,
    `\n| maxFeePerGas\t\t| ${tx.maxFeePerGas?.toString()}\t\t\t|`,
    `\n| totalCost\t\t| ${formatEther(
      receipt.gasUsed.mul((tx as any).maxFeePerGas)
    )} ETH\t\t|`,
    // refund
  ...
  .setAction(async ({ contractName, args }, hre) => {
    ...
    try {
      ...
      const argsFile = join(
        ...
        "..",
        "args-" + contractName + "@" + deployed.address + ".json"
      );
